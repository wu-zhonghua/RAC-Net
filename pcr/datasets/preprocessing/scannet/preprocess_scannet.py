...
def handle_process(scene_path, output_path, labels_pd, train_scenes, val_scenes, points_20, parse_normals=True):
    ...
    if split_name != "test":
        ...
        if split_name == 'train':
            ...
            # Generate new labels
            semantic_gt20 = np.ones((vertices.shape[0], 1)) * IGNORE_INDEX
            semantic_gt20_final = np.ones((vertices.shape[0], 1)) * IGNORE_INDEX
            semantic_gt200 = np.ones((vertices.shape[0], 1)) * IGNORE_INDEX
            instance_ids = np.ones((vertices.shape[0], 1)) * IGNORE_INDEX
            for group in seg_groups:
                point_idx, label_id20, label_id200 = \
            ...
            semantic_gt20_final[points_20[scene_id]] = semantic_gt20[points_20[scene_id]]
            semantic_gt20 = semantic_gt20_final
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_root', default='/mnt/scannet/', help='Path to the ScanNet dataset containing scene folders')
    parser.add_argument('--output_root', default='/mnt/scannet_20points/', help='Output path where train/val folders will be located')
    ...
    points_20 = torch.load('pcr/datasets/preprocessing/scannet/points20')
    ...
    _ = list(pool.map(handle_process, scene_paths, repeat(config.output_root), repeat(labels_pd), repeat(train_scenes),
                      repeat(val_scenes), repeat(points_20), repeat(config.parse_normals)))
