...
def default_argument_parser(epilog=None):
    ...
    parser.add_argument('--config-file', default="configs/scannet/semseg-ptv2m2-0-base.py", metavar="FILE", help="path to config file")
...
class Trainer:
    ...
    def run_step(self, i, input_dict):
        ...
        with torch.cuda.amp.autocast(enabled=self.cfg.enable_amp):
            ...
            lds_cda = 0
            loss = 0
            output = self.model(input_dict["coord"], input_dict["feat"], input_dict["offset"].int())
            ...
            try:
                ...
                #     # calc adversarial direction
                    pred_hat_wolf = self.model(input_dict["coord_pointwolf"], input_dict["feat_pointwolf"], input_dict["offset"].int())
                    out_prob.append(F.softmax(pred_hat_wolf, dim=1))
                    logp_hat_wolf = F.log_softmax(pred_hat_wolf, dim=1)
                    
                    pred_hat_cda = self.model(input_dict["coord_at"], input_dict["feat_at"], input_dict["offset"].int())
                ...
                if len(pseudo_label[selected_idx]) > 0:
                    ...
                    d = torch.rand(input_dict["coord_at"].shape[0]).to(input_dict["coord_at"].device)
                    mix_features = copy.deepcopy(input_dict["feat_at"])
                    mix_points = input_dict["coord_at"] * d.unsqueeze(1) + input_dict["coord_pointwolf"] * (1 - d.unsqueeze(1))
                    mix_features[:, 0:3] = mix_points
                    
                    # mix_batch = generate_batch(mix_batch, mix_points)
                    with _disable_tracking_bn_stats(self.model):
                        pred_hat_mix = self.model(mix_points, mix_features, input_dict["offset"].int())
                    ...
                    total_loss = total_loss + loss_wolf_pseudo + loss_cda_pseudo + lds_wolf + lds_cda + lds_mix
                    length = len(pseudo_label[selected_idx])
                else:
                    loss_wolf_pseudo = 0
                    loss_cda_pseudo = 0
                    ...
                    total_loss = total_loss + lds_wolf + lds_cda
            # end consistency training
            except:
                # output = self.model(input_dict["coord"], input_dict["feat"], input_dict["offset"].int())
                # loss = self.criteria(output, input_dict["label"])
                # total_loss = loss
                print('forward out of memory')
        ...
        if comm.get_world_size() > 1:
            ...
            # try:
            dist.all_reduce(total_loss), dist.all_reduce(count)
            # except:
            #     print('reduce error')
            n = count.item()
            total_loss /= n
