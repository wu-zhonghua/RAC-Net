...
def generate_batch(batch, new_points):
    ...
    for i in range(len(batch.points)):
        ...
        batch.lengths[i] = torch.tensor(batch.lengths[i])
        if i < 4:
            batch.pools[i] = torch.tensor(batch.pools[i].astype(np.int64)).to(batch.features.device)
            batch.upsamples[i] = torch.tensor(batch.upsamples[i].astype(np.int64)).to(batch.features.device)
...
def S3DISCollate(batch_data):
    ...
    data_3 = S3DISCustomBatch(copy.deepcopy(batch_data))
     ...
     # wolf
    final_mix_point = None
    for i in range(len(data_1.lengths[0])):
        if i == 0:
            ori_points = data_1.points[0][0:data_1.lengths[0][i], :]
        elif i == 1:
            ori_points = data_1.points[0][data_1.lengths[0][i-1]:(data_1.lengths[0][i-1]+data_1.lengths[0][i]), :]
        elif i == 2: 
            ori_points = data_1.points[0][data_1.lengths[0][i-2] + data_1.lengths[0][i-1]:(data_1.lengths[0][i-2] + data_1.lengths[0][i-1]+data_1.lengths[0][i]), :]  
        elif i == 3:
            ori_points = data_1.points[0][data_1.lengths[0][i-3] + data_1.lengths[0][i-2] + data_1.lengths[0][i-1]:(data_1.lengths[0][i-3] + data_1.lengths[0][i-2] + data_1.lengths[0][i-1]+data_1.lengths[0][i]), :]  
        elif i == 4:
            ori_points = data_1.points[0][data_1.lengths[0][i-4] + data_1.lengths[0][i-3] + data_1.lengths[0][i-2] + data_1.lengths[0][i-1]:(data_1.lengths[0][i-4] + data_1.lengths[0][i-3] + data_1.lengths[0][i-2] + data_1.lengths[0][i-1]+data_1.lengths[0][i]), :]
        elif i == 5:
            ori_points = data_1.points[0][data_1.lengths[0][i-5] + data_1.lengths[0][i-4] + data_1.lengths[0][i-3] + data_1.lengths[0][i-2] + data_1.lengths[0][i-1]:(data_1.lengths[0][i-5] + data_1.lengths[0][i-4] + data_1.lengths[0][i-3] + data_1.lengths[0][i-2] + data_1.lengths[0][i-1]+data_1.lengths[0][i]), :]
        else:
            print('what?????????')
            exit()
        
        _, mix_point = pointwolf(ori_points.cpu().numpy())
        mix_point = torch.tensor(mix_point)
        
        if i == 0:
            final_mix_point = mix_point
        else:
            final_mix_point = torch.cat((final_mix_point, mix_point), dim=0)
            
    data_2 = generate_batch(data_2, final_mix_point)
    ...
    final_mix_point = None
    for i in range(len(data_1.lengths[0])):
        ori_points = data_1.points[0][i0: i0 + data_1.lengths[0][i], :]
        i0 += data_1.lengths[0][i]
        
        mix_point = pointwolf.local_transformaton(ori_points.unsqueeze(0).cpu().numpy())
        mix_point = torch.tensor(mix_point).squeeze(0)
    ...
    data_3 = generate_batch(data_3, final_mix_point)    
