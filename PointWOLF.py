...
class PointWOLF(object):
    def __init__(self, w_num_anchor=4, w_sample_type='fps', w_sigma=0.5, w_R_range=10, w_S_range=3, w_T_range=0.25):
    ...
    def __call__(self, pos):
        ...
        # my new normalize
        pos_new = pos_new - pos_new.mean(axis=-2, keepdims=True)
        scale = (np.sqrt((pos ** 2).sum(1)).max() / np.sqrt((pos_new ** 2).sum(1)).max()) * 0.999999
        pos_new = scale * pos_new
